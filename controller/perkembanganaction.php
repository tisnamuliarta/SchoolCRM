...
if (isset($_POST['btn_action_nilai_perkembangan'])) {
    if ($_POST['btn_action_nilai_perkembangan'] == 'load_nama_siswa') {
     ...
     * ==========================================
     */
    if ($_POST['btn_action_nilai_perkembangan'] == 'Add') {
        $nis = $_POST['nis'];
        // select perkembangan
        ...
        if ($count > 0) {
            ...
            $query = "
				INSERT INTO tb_perkembangan (nip,nis,pembiasaan,bahasa,daya_fikir,motorik,tgl,id_kelas,id_tahun_ajaran,id_kegiatan) 
				VALUES (:nip,:nis,:pembiasaan,:bahasa,:daya_fikir,:motorik,:tgl,:id_kelas,:id_tahun_ajaran,:id_kegiatan)
			";
     ...
     * ====================================
     */
    if ($_POST['btn_action_nilai_perkembangan'] == 'fetch_single') {
        $query = " SELECT tb_perkembangan.*, tb_siswa.nama, tb_siswa.jenis_kelamin, 
            DATE_FORMAT(tb_siswa.tgl_lahir,'%d %M %Y') as tanggal_lahir ,
			...
			WHERE tb_siswa.nis IS NOT NULL AND tb_perkembangan.nis = :nis 
			AND tb_perkembangan.id_kelas = :id_kelas AND tb_perkembangan.id_kegiatan = :id_kegiatan ";
        $statement = $connect->prepare($query);
        $statement->execute([
     ...
     * ==================================================
     * */
    if ($_POST['btn_action_nilai_perkembangan'] == 'Edit') {
        $query = "
			UPDATE tb_perkembangan
			set daya_fikir = :daya_fikir,
			bahasa = :bahasa,
			pembiasaan = :pembiasaan,
			motorik = :motorik
			WHERE nis = :nis AND id_kegiatan = :id_kegiatan AND id_kelas = :id_kelas
		";
        $statement = $connect->prepare($query);
        $statement->execute(
            array(
     ...
     * ===============================
     */
    if ($_POST['btn_action_nilai_perkembangan'] == 'delete') {
        $query ="
